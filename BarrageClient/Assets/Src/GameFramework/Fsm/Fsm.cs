    ...
    public  class Fsm<T> : FsmBase, IFsm<T> where T : class
    {
            ...
            : base(name)
        {
            ...
            foreach (FsmState<T> state in states)
            {
                ...
                {
                    throw new GameFrameworkException(string.Format("FSM '{0}' state '{1}' is already exist.", TextHelper.GetFullName<T>(name), stateName));
                }
        ...
        public void Start<TState>() where TState : FsmState<T>
        {
            ...
            if (state == null)
            {
                throw new GameFrameworkException(string.Format("FSM '{0}' can not start state '{1}' which is not exist.", TextHelper.GetFullName<T>(Name), typeof(TState).FullName));
            }
        ...
        public TState CreateFsmState<TState>() where TState : FsmState<T> ,new()
        {

            TState state = Activator.CreateInstance(typeof(TState)) as TState;
            return CreateFsmState(state);
        }
    
         
        public TState CreateFsmState<TState>(TState fsmstate) where TState : FsmState<T>
        {
            if (fsmstate == null)
            {
        ...
        public void Start(Type stateType)
        {
            ...
            if (state == null)
            {
                throw new GameFrameworkException(string.Format("FSM '{0}' can not start state '{1}' which is not exist.", TextHelper.GetFullName<T>(Name), stateType.FullName));
            }
        ...
        public void ChangeState(Type stateType)
        {
            ...
            if (state == null)
            {
                throw new GameFrameworkException(string.Format("FSM '{0}' can not change state to '{1}' which is not exist.", TextHelper.GetFullName<T>(Name), stateType.FullName));
            }
